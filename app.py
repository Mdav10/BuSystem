...
def dashboard():
    ...
    total_assets_value = db.session.query(func.sum(Asset.current_value)).filter(Asset.user_id == user_id).scalar() or 0
    ...
    alerts = []
    for animal in Livestock.query.filter(Livestock.user_id == user_id, Livestock.status == 'Active', Livestock.expected_sell_date <= today).limit(5).all():
...
def get_budget():
    ...
    return jsonify([{
        'id': b.id, 'category': b.category, 'type': b.type,
        'expected_amount': b.expected_amount, 'actual_amount': b.actual_amount,
...
def set_budget():
    data = request.json
    today = datetime.now()
    budget = Budget.query.filter_by(user_id=current_user.id, category=data.get('category'), month=today.month, year=today.year).first()
    if budget:
        budget.expected_amount = float(data.get('expected_amount'))
    else:
        budget = Budget(
            user_id=current_user.id, category=data.get('category'),
            type=data.get('type'), expected_amount=float(data.get('expected_amount')),
...
def get_decisions():
    ...
    for b in over_budget[:3]:
        recommendations.append({
            ...
            'message': f'Exceeded by {b.actual_amount - b.expected_amount:,.0f} {current_user.currency}',
            'type': 'warning'
        })
...
def get_timeline():
    ...
    for t in Transaction.query.filter_by(user_id=user_id).order_by(Transaction.date.desc()).limit(50).all():
        events.append({'date': t.date.strftime('%Y-%m-%d'), 'type': 'transaction', 'title': f"{t.type}: {t.category}", 'description': f"{t.amount:,.0f} {current_user.currency}", 'icon': '💰'})
    for i in Investment.query.filter_by(user_id=user_id).order_by(Investment.purchase_date.desc()).limit(30).all():
        events.append({'date': i.purchase_date.strftime('%Y-%m-%d'), 'type': 'investment', 'title': f"Investment: {i.investment_id}", 'description': f"{i.capital:,.0f} {current_user.currency}", 'icon': '📊'})
...
def get_rules():
    ...
    return jsonify([{
        ...
        'condition_operator': r.condition_operator, 'action_message': r.action_message
    } for r in rules])
...
def add_rule():
    ...
    rule = FinancialRule(
        ...
        condition_type=data.get('condition_type'), condition_value=float(data.get('condition_value')),
        condition_operator=data.get('condition_operator'), action_type=data.get('action_type', 'warn'),
...
def check_rules():
    ...
    for rule in rules:
        if rule.category == 'investment':
            investments = Investment.query.filter_by(user_id=current_user.id, status='Running').all()
            ...
            if total_capital > 0:
                for inv in investments:
                    ...
                    if rule.condition_operator == '>' and percentage > rule.condition_value:
                        alerts.append(f"⚠️ {rule.name}: {inv.type} exceeds {rule.condition_value}%")
            ...
            if rule.condition_operator == '>' and spending_ratio > rule.condition_value:
                alerts.append(f"⚠️ {rule.name}: Spending at {spending_ratio:.1f}%")
...
def get_report_data(report_type):
    ...
    if report_type == 'income_statement':
        income = db.session.query(func.sum(Transaction.amount).label('total'), Transaction.category).filter(
            Transaction.user_id == user_id, Transaction.type == 'income'
        ).group_by(Transaction.category).all()
        expenses = db.session.query(func.sum(Transaction.amount).label('total'), Transaction.category).filter(
            Transaction.user_id == user_id, Transaction.type == 'expense'
        ).group_by(Transaction.category).all()
        return jsonify({
