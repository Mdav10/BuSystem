...
# ============================
# COMPLETE BUDGET API - FIXED TRACKING
# ============================
...
def api_budget():
    ...
    if request.method == 'GET':
        ...
        transaction = Transaction(
            ...
            description=f"Budget reserved: {data.get('name')} - {data.get('category')}",
            date=datetime.utcnow()
        )
        db.session.add(transaction)
        db.session.commit()
        ...
        if budget.is_cash_reserved and budget.status != 'completed':
            refund_amount = budget.planned_amount - budget.actual_amount
            if refund_amount > 0:
                transaction = Transaction(
                    user_id=user_id,
                    type='income',
        ...
        return jsonify({
            'status': 'success',
            'message': 'Budget deleted and remaining cash refunded',
            'current_cash': new_cash
        })
...
def track_budget_spending(id):
    """Track actual spending against a budget - NO CASH DEDUCTION (already deducted when budget was created)"""
    ...
    if amount > remaining:
        return jsonify({
            'error': f'Insufficient budget remaining! You have {remaining:,.0f} BIF left in this budget.',
            'remaining': remaining,
            'required': amount
        }), 400
    
    # Update budget - NO CASH DEDUCTION (cash already deducted when budget was created)
    ...
    # ===== DO NOT CREATE EXPENSE TRANSACTION - CASH ALREADY DEDUCTED =====
    # Just update the budget tracking
    
    # Get current cash (for display purposes only)
    total_income = db.session.query(func.sum(Transaction.amount)).filter(
    ...
    return jsonify({
        ...
        'current_cash': current_cash,
        'amount_spent': amount,
        'message': f'✅ {amount:,.0f} BIF tracked from "{budget.name}". Remaining budget: {budget.remaining_amount:,.0f} BIF'
    })
...
# ============================
# BUDGET SUMMARY - USING SAME PATTERN
# ============================
...
def budget_summary():
    ...
    try:
        ...
        # ===== SAME CASH CALCULATION AS INVESTMENTS =====
        total_income = db.session.query(func.sum(Transaction.amount)).filter(
            Transaction.user_id == user_id,
            Transaction.type == 'income'
        ).scalar() or 0
...
# ============================
# INCOME STATEMENT EXPORT - SUPERADMIN ONLY
# ============================
...
def export_income_statement(format):
    ...
    if format == 'pdf':
        ...
        workbook.close()
        output.seek(0)
        return send_file(output, as_attachment=True, download_name=f"Income_Statement_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx")
    
    return jsonify({'error': 'Invalid format'}), 400


# ============================
# BALANCE SHEET EXPORT - SUPERADMIN ONLY
# ============================
...
def export_balance_sheet(format):
    ...
    if format == 'pdf':
        ...
        workbook.close()
        output.seek(0)
        return send_file(output, as_attachment=True, download_name=f"Balance_Sheet_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx")
...
# ============================
# REPORTS DATA ROUTES - FOR REPORTS PAGE (SUPERADMIN)
# ============================
...
def get_income_statement_data():
    """Get income statement data for Reports page"""
    user_id = current_user.id
    ...
    return jsonify({
        ...
        'income_by_category': [{'category': i[0], 'amount': float(i[1])} for i in income_by_category],
        'expenses_by_category': [{'category': i[0], 'amount': float(i[1])} for i in expenses_by_category]
    })
...
def get_balance_sheet_data():
    """Get balance sheet data for Reports page"""
    user_id = current_user.id
    ...
    return jsonify({
        ...
        'net_worth': float(total_assets + total_income - total_expenses + total_owed_to_me - total_liabilities)
    })
